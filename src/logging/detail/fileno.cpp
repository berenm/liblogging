...
#  error "Does anybody know how to fetch the bloody file descriptor?"
    ...
    template< >
    int fileno< char > (::std::ios const& stream_in) {
      return fileno_hack(stream_in);
    }

    template< >
    bool isatty< char > (::std::ios const& stream_in) {
      int f = ::logging::detail::fileno(stream_in);
      return !(f < 0) && ::isatty(f);
    }
    ...
    template< >
    int fileno< wchar_t > (::std::wios const& stream_in) {
      return fileno_hack(stream_in);
    }

    template< >
    bool isatty< wchar_t > (::std::wios const& stream_in) {
      int f = ::logging::detail::fileno(stream_in);
      return !(f < 0) && ::isatty(f);
    }
#endif
